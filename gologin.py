...
class GoLogin(object):
    def __init__(self, options):
        ...
        self.port  = options.get('port', options.get("NumberPortTool"))
    ...
    def start(self):
        ...
        print("start profile_path:"+profile_path)
    ...
    def downloadProfileZip(self):
        ...
        if s3path=='':
            ...
            urlDownload = API_URL + '/browser/'+self.profile_id
            print(urlDownload+"\n"+headers)
            data = requests.get(urlDownload, headers=headers).content
        else:
...
##        splitPathFile =self.profile_path.split("Temp\\")
##        ROOT_DIR = os.path.abspath(os.curdir)+"\TaoProfileGologin\\ProfileGologin\\"+splitPathFile[1]
##        if not os.path.exists(os.path.join(ROOT_DIR, 'Default', 'Preferences')):
##            self.uploadEmptyProfile()
##            self.createEmptyProfile()
##            self.extractProfileZip()
...
##        splitPathFile =self.profile_path.split("Temp\\")
##        ROOT_DIR = os.path.abspath(os.curdir)+"\TaoProfileGologin\\ProfileGologin\\"+splitPathFile[1]
##        shutil.copy(empty_profile, ROOT_DIR)
    ...
    def extractProfileZip(self):
        with zipfile.ZipFile(self.profile_zip_path, 'r') as zip_ref:
            ...
            splitPathFile =self.profile_path.split("Temp\\")
            ROOT_DIR = os.path.abspath(os.curdir)+"\TaoProfileGologin\\ProfileGologin\\"+splitPathFile[1]
            print("zip_ref.extractall: "+ROOT_DIR)
    ...
    def createStartup(self):
        print("createStartup profile path: "+self.profile_path)
        print("createStarup: "+str(self.local))
    ...
    def create(self, options={}, ipLocal=999):

        profile_options = self.getRandomFingerprint(options)
        #print("profile_options: "+str(profile_options.get("navigator", {})))
        #print("profile_options: "+str(profile_options.get("webGLMetadata")))
        ...
        profile = {
          ...
          "webRTC": {
            ...
            "fillBasedOnIp": False,
            "localIpMasking": True,
            "publicIp": "192.168.1."+str(ipLocal),
            "localIps": []
          ...
          "webGLMetadata":{
            "mode": "mask",
            "vendor": profile_options.get("webGLMetadata")["vendor"],
            "renderer": profile_options.get("webGLMetadata")["renderer"]
          },
##          "storage": {
##            "local": True,
##            "extensions": True,
...
##           profile_options.get('webGLMetadata', {})
##          "screenHeight": objScreen.ScreenHeight,
##          "screenWidth": objScreen.ScreenWith,
          "proxyEnabled": False,
          "profile": json.dumps(profile_options),
        }

##        if profile.get('navigator'):
##          profile['navigator']['resolution'] = objScreen.ScreenDisplay
##        else:
##          profile['navigator'] = {'resolution': objScreen.ScreenDisplay}
